$(() => {
  window.cc = new Bitbucket({
    ...
    enterprise: '',
    debug_url:
      'https://bitbucket.org/osallou/go-docker/src/7766b261f7cefae31688636b830fd3497fc80c05/godocker/godscheduler.py',
  });
});
...
describe('bitbucket src', () => {
  ...
  it('should add covered lines', () => {
    ...
    expect($("a[name='godscheduler.py-70']").hasClass('codecov codecov-hit')).to
      .be.true;
    expect(
      $("a[name='godscheduler.py-71']").hasClass('codecov codecov-partial'),
    ).to.be.true;
    expect($("a[name='godscheduler.py-74']").hasClass('codecov')).to.be.false;
    expect($("a[name='godscheduler.py-75']").hasClass('codecov codecov-missed'))
      .to.be.true;
  });
  it('will toggle it', () => {
    // first click
    ...
    expect($("a[name='godscheduler.py-70']").hasClass('codecov-on')).to.be
      .false;
    expect($("a[name='godscheduler.py-71']").hasClass('codecov-on')).to.be
      .false;
    expect($("a[name='godscheduler.py-75']").hasClass('codecov-on')).to.be
      .false;
    // second click
    click($('.codecov.aui-button')[0]);
    expect($('.codecov.aui-button').hasClass('aui-button-light')).to.be.true;
