...
const Cls = (window.Codecov = class Codecov {
  ...
  log(title, data) { if (this.settings.debug) { return console.log(this, title, data); } }
    ...
    */
    ...
    return $.ajax({
      ...
      error(xhr, type, reason) {
        ...
        if (self.settings.urls.length > (self.urlid+1)) { return self._get(self.settings.urls[(self.urlid+=1)]); }
      }
    });
  }
  ...
  color(ln) {
    if ((ln == null)) { return; }  // undefined or null
    const c = (ln.c == null) ? ln : ln.c;
    if (c === 0) {
      return "missed";
    } else if (c === true) {
      return "partial";
    } else if (Array.from(c).includes('/')) {
      const v = c.split('/');
      if (v[0] === '0') {
        return "missed";
      } else if (v[0] === v[1]) {
  ...
  _validate_codecov_yml() {
    ...
    if (yml) {
      return $.ajax({
        ...
        error(xhr, type, reason) { return self.yml_error(reason); }
      });
    }
  }
});
...
window.create_codecov_instance = function(prefs, cb) {
  ...
  } else if ((needle = $('meta[name="application-name"]').attr('content'), ['Bitbucket', 'Stash'].includes(needle))) {
    return new Bitbucket(prefs, cb);

  } else if ((needle1 = 'GitLab', Array.from($('meta[name="description"]').attr('content')).includes(needle1))) {
