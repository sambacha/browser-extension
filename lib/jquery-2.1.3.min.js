...
!(function (a, b) {
  ...
  const t = (function (a) {
    ...
    const ab = /[+~]/;
    const bb = /'|\\/g;
    const cb = new RegExp(`\\\\([\\da-f]{1,6}${L}?|(${L})|.)`, 'ig');
    ...
    return (
      ...
      jb((a) => a.getAttribute('disabled') == null) ||
        kb(K, (a, b, c) => {
          let d;
  ...
  const ab = /<(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^>]*)\/>/gi;
  const bb = /<([\w:]+)/;
  const cb = /<|&#?\w+;/;
  ...
  const fc = /([?&])_=[^&]*/;
  const gc = /^(.*?):[ \t]*([^\r\n]*)$/gm;
  const hc = /^(?:about|app|app-storage|.+-extension|file|res|widget):$/;
  const ic = /^(?:GET|HEAD)$/;
  const jc = /^\/\//;
  const kc = /^([\w.+-]+:)(?:\/\/(?:[^\/?#]*@|)([^\/?#:]*)(?::(\d+)|)|)/;
  const lc = {};
  const mc = {};
  const nc = '*/'.concat('*');
  const oc = a.location.href;
  const pc = kc.exec(oc.toLowerCase()) || [];
  ...
  const wc = /\[\]$/;
  const xc = /\r?\n/g;
  const yc = /^(?:submit|button|image|reset|file)$/i;
  const zc = /^(?:input|select|textarea|keygen)/i;
  ...
  let Bc = 0;
  const Cc = {};
  const Dc = { 0: 200, 1223: 204 };
  let Ec = n.ajaxSettings.xhr();
