  ...
  Codecov.prototype._process = function (res, store) {
    ...
    this.yaml = {
      ...
      range: [
        ...
        parseFloat(
          ((ref9 = res.repo) != null
            ? (ref10 = ref9.yaml) != null
              ? (ref11 = ref10.coverage) != null
                ? ref11.range
                : void 0
              : void 0
            : void 0) != null
...
window.create_codecov_instance = function (prefs, cb) {
  ...
  if (
    (ref2 = $('meta[name="application-name"]').attr('content')) ===
      'Bitbucket' ||
    ref2 === 'Stash'
  ) {
  ...
  if (
    indexOf.call($('meta[name="description"]').attr('content'), 'GitLab') >= 0
  ) {
...
window.Github = (function (superClass) {
  ...
  Github.prototype.get_ref = function (href) {
    ...
    if (this.page === 'commits') {
      ...
      return `branch/${
        href[6] || $('.branch-select-menu .js-select-button').text()
      }/commits?start=${from}&stop=${to}`;
    }
    if (href[7] === 'commits') {
      this.page = 'commits';
      to = $('li.commit:first a:last').attr('href').split('/')[4];
    ...
    if ((ref1 = this.page) === 'blob' || ref1 === 'blame') {
    ...
    if (this.page === 'compare') {
      this.base = `${
        $('.commit-id:first').text() ||
        $('input[name=comparison_start_oid]').val()
      }`;
      return (
        $('.commit-id:last').text() || $('input[name=comparison_end_oid]').val()
      );
    }
    if (this.page === 'pull') {
      this.base = `${
    ...
    $('.repository-content .file').each(function () {
      ...
      if (file.find('.btn.codecov').length === 0) {
        ...
        return file
          ...
          .prepend(
            '<a class="btn BtnGroup-item btn-sm codecov disabled tooltipped tooltipped-n"\naria-label="Requesting coverage from Codecov.io"\ndata-hotkey="c">Coverage loading...</a>',
          )
          .find('.btn')
          .addClass('BtnGroup-item');
      }
    });
    return true;
  };
  ...
  Github.prototype.overlay = function (res) {
    ...
    if (this.page === 'commits') {
      ...
      base_url = `${self.settings.urls[self.urlid]}/${self.service}/${
        self.slug
      }/commit`;
      return $('li.commit').each(function () {
        ...
        if (
          ...
          return $commit
            ...
            .append(
              `<a title="Codecov Coverage" href="${base_url}/${
                commit.commitid
              }">${self.format(commit.totals.c)}%</a>`,
            );
        }
      });
    }
    report =
      ((ref1 = res.commit) != null ? ref1.report : void 0) ||
      ((ref2 = res.head) != null ? ref2.report : void 0) ||
      res.report;
    if (this.page === 'tree') {
      total =
        ((ref3 = report.totals) != null ? ref3.c : void 0) != null
          ? report.totals.c
          : report.coverage;
      $('.commit-tease span.right').append(
        `<a href="${this.settings.urls[this.urlid]}/github/${this.slug}?ref=${
          this.ref
      ...
      return $('.file-wrap tr:not(.warning):not(.up-tree)').each(function () {
        ...
        filepath =
          (ref4 = $('td.content a', this).attr('href')) != null
            ? ref4.split('/').slice(5).join('/')
            : void 0;
        if (filepath) {
          ...
          if (file != null) {
            ...
            cov =
              ((ref6 = file.t) != null ? ref6.c : void 0) != null
                ? file.t.c
                : file.coverage;
            if (cov != null) {
              path =
                ((ref7 = file.t) != null ? ref7.c : void 0) != null
                  ? `src/${self.ref}/${filepath}`
                  : `${filepath}?ref=${self.ref}`;
              return $('td:last', this).after(
                `<td style="background:linear-gradient(90deg, ${self.bg(
                  cov,
                )} ${cov}%, white ${cov}%);text-align:right;"\n    class="sha codecov codecov-removable tooltipped tooltipped-n"\n    aria-label="Codecov Coverage">\n  <a href="${
                  self.settings.urls[self.urlid]
                }/${self.service}/${self.slug}/${path}">\n    ${self.format(
                  cov,
                )}%\n  </a>\n</td>`,
              );
            }
    ...
    if (
      (ref4 = this.page) === 'commit' ||
      ref4 === 'compare' ||
      ref4 === 'pull'
    ) {
      $('.toc-diff-stats, .diffbar-item.diffstat, #diffstat').append(
        `<span class="codecov codecov-removable"> <strong>${self.format(
          report.totals.c,
        )}%</strong></span>`,
      );
    }
    ...
    $('.repository-content .file').each(function () {
      ...
      if (fp) {
        ...
        if (
          file_data != null &&
          (file_data != null ? file_data.ignored : void 0) !== true
        ) {
          total = self.format(
            ((ref5 = file_data.t) != null ? ref5.c : void 0) != null
              ? file_data.t.c
              : file_data.coverage,
          );
          button = file
            ...
            .attr(
              'aria-label',
              'Toggle Codecov (c), alt+click to open in Codecov',
            )
            .attr(
              'data-codecov-url',
              `${self.settings.urls[self.urlid]}/${self.service}/${self.slug}/${
                ((ref7 = file_data.t) != null ? ref7.c : void 0) != null
                  ? `src/${self.ref}/${fp}`
                  : `${fp}?ref=${self.ref}`
              }`,
            )
            ...
            .unbind()
            .click(
              (ref6 = self.page) === 'blob' || ref6 === 'blame'
          ...
          file.find('tr').each(function () {
            ...
            cov = self.color(
              file_lines[
                td.attr('data-line-number') ||
                  ((ref8 = td.attr('id')) != null ? ref8.slice(1) : void 0)
              ],
            );
            if (cov) {
              if (split_view) {
          ...
          if ((ref8 = self.page) === 'commit' || ref8 === 'pull') {
            ...
            if (self.page === 'pull') {
              $(`a[href="#${file.prev().attr('name')}"] .diffstat`).prepend(
                `<span class="codecov codecov-removable">${total}% <strong>(${diff}%)</strong></span>`,
              );
            } else {
          ...
          if (
            ...
            ((ref9 = self.page) === 'blob' || ref9 === 'blame')
          ) {
      ...
      $('.btn.codecov')
        ...
        .click(
          () =>
            (window.location = `${
              self.settings.urls[self.urlid]
            }/login/github?redirect=${escape(window.location.href)}`),
        );
...
window.Bitbucket = (function (superClass) {
  ...
  Bitbucket.prototype.get_ref = function (href) {
    ...
    if (this.page === 'pull-requests') {
      return (ref = $('.view-file:first').attr('href')) != null
        ? ref.split('/')[4]
        : void 0;
    }
    return false;
  ...
  Bitbucket.prototype.prepare = function () {
    ...
    $('#editor-container, section.bb-udiff').each(function () {
      if ($('.aui-button.codecov', this).length === 0) {
        return $('.secondary>.aui-buttons:first', this).prepend(
          '<a href="#" class="aui-button aui-button-light codecov"\ntitle="Requesting coverage from Codecov.io">Coverage loading...</a>',
        );
      }
    });
    return true;
  ...
  Bitbucket.prototype.overlay = function (res) {
    ...
    $('#source-list tr').each(function () {
      ...
      fp =
        (ref2 = $('a', this).attr('href')) != null
          ? ref2.split('?')[0].split('/').slice(5).join('/')
          : void 0;
      cov =
        report != null
          ? (ref3 = report.files) != null
            ? (ref4 = ref3[fp]) != null
              ? ref4.t.c
              : void 0
            : void 0
    ...
    $('section.bb-udiff').each(function () {
      ...
      data =
        report != null
          ? (ref2 = report.files) != null
            ? ref2[fp]
            : void 0
          : void 0;
      if (data != null) {
        button = $('.aui-button.codecov', this)
