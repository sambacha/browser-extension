...
window.Codecov = (function () {
  ...
  Codecov.prototype._process = function (res, store) {
    let error; let obj; let ref; let ref1; let ref10; let ref11; let ref2; let ref3; let ref4; let ref5; let ref6; let ref7; let ref8; let
      ref9;
...
window.create_codecov_instance = function (prefs, cb) {
  ...
  } if ((ref2 = $('meta[name="application-name"]').attr('content')) === 'Bitbucket' || ref2 === 'Stash') {
    return new Bitbucket(prefs, cb);
  } if (indexOf.call($('meta[name="description"]').attr('content'), 'GitLab') >= 0) {
...
window.Github = (function (superClass) {
  ...
  Github.prototype.get_ref = function (href) {
    let from; let ref; let ref1; let split; let
      to;
  ...
  Github.prototype.overlay = function (res) {
    let _td; let base_url; let c; let commits; let i; let len; let ref; let ref1; let ref2; let ref3; let ref4; let ref5; let report; let self; let split_view; let total; let total_hits; let
      total_lines;
    ...
    if (this.page === 'tree') {
      ...
      return $('.file-wrap tr:not(.warning):not(.up-tree)').each(function () {
        let cov; let file; let filepath; let path; let ref4; let ref5; let ref6; let
          ref7;
    ...
    $('.repository-content .file').each(function () {
      let button; let diff; let file; let file_data; let file_lines; let fp; let hits; let lines; let ref5; let ref6; let ref7; let ref8; let
        ref9;
      ...
      if (fp) {
        ...
        if ((file_data != null) && (file_data != null ? file_data.ignored : void 0) !== true) {
          ...
          button = file.find('.btn.codecov').attr('aria-label', 'Toggle Codecov (c), alt+click to open in Codecov').attr('data-codecov-url', `${self.settings.urls[self.urlid]}/${self.service}/${self.slug}/${((ref7 = file_data.t) != null ? ref7.c : void 0) != null ? `src/${self.ref}/${fp}` : `${fp}?ref=${self.ref}`}`).text(`${total}%`)
            .removeClass('disabled')
            .unbind()
            .click((ref6 = self.page) === 'blob' || ref6 === 'blame' ? self.toggle_coverage : self.toggle_diff);
          hits = 0;
          lines = 0;
          file_lines = file_data.l != null ? file_data.l : file_data.lines;
          file.find('tr').each(function () {
            let cov; let ref8; let
              td;
  ...
  Github.prototype.error = function (status, reason) {
    if (status === 401) {
      $('.btn.codecov').text('Please login at Codecov').addClass('danger').attr('aria-label', 'Login to view coverage by Codecov')
        .click(() => window.location = `${self.settings.urls[self.urlid]}/login/github?redirect=${escape(window.location.href)}`);
      return $('.commit.codecov .sha-block').addClass('tooltipped tooltipped-n').text('Please login into Codecov').attr('aria-label', 'Login to view coverage by Codecov')
        .click(() => window.location = `${self.settings.urls[self.urlid]}/login/github?redirect=${escape(window.location.href)}`);
    } if (status === 404) {
...
window.Bitbucket = (function (superClass) {
  ...
  Bitbucket.prototype.overlay = function (res) {
    let ref; let ref1; let report; let
      self;
    ...
    $('#source-list tr').each(function () {
      let cov; let fp; let ref2; let ref3; let
        ref4;
    ...
    $('section.bb-udiff').each(function () {
      let $file; let button; let data; let fp; let
        ref2;
      ...
      if (data != null) {
        button = $('.aui-button.codecov', this).attr('title', 'Toggle Codecov').text(`Coverage ${self.format(data.t.c)}%`).attr('data-codecov-url', `${self.settings.urls[self.urlid]}/${self.service}/${self.slug}/src/${self.ref}/${fp}`)
          .unbind()
    ...
    return $('#editor-container').each(function () {
      let $file; let button; let cov; let file; let filename; let fp; let ln; let ref2; let ref3; let
        ref4;
      ...
      if (file != null) {
        button = $file.find('.aui-button.codecov').attr('title', 'Toggle Codecov').text(`Coverage ${self.format(file.t.c)}%`).attr('data-codecov-url', '[TODO]')
          .unbind()
  ...
  Bitbucket.prototype.error = function (status, reason) {
    if (status === 401) {
      return $('.aui-button.codecov').text('Please login at Codecov').addClass('aui-button-primary').attr('title', 'Login to view coverage by Codecov')
